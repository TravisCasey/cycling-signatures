...
def test_build_and_query(square_loop_points):
    ...
    storage = cs.CycleStorage.build(embedded, range(0, count), count, threshold=1.0)
...
def test_save_load_roundtrip(tmp_path, square_loop_points):
    embedded = cs.EmbeddedTrajectory(cs.Trajectory(square_loop_points), cs.Euclidean())
    count = square_loop_points.shape[0]
    storage = cs.CycleStorage.build(embedded, (0, count), count, threshold=1.0)
...
def test_build_with_omitted_threshold_reports_adjacency_bound(square_loop_points):
    embedded = cs.EmbeddedTrajectory(cs.Trajectory(square_loop_points), cs.Euclidean())
    count = square_loop_points.shape[0]
