...
"""Dominance and purity maps (Dadras)
=====================================

Every query point on the Dadras attractor carries a *dominant* signature:
...
# signatures are computed on every ``LABEL_STEP``-th detection point, and
# unlabeled points are ignored by the neighborhood tallies. ``MIN_NEIGHBORS``
# is the minimum count of labeled neighbors to trust a dominance reading;
# points with fewer are skipped. ``QUERY_STEP`` and ``BACKGROUND_STEP`` stride
# the raw rows the two scatters draw, keeping the figures legible; both are
# counted in raw rows, which are denser than the detection points.
...
# %%
# **Label detection points on a stride.** For every ``LABEL_STEP``-th point
# in the labeled prefix (the range where a full window fits), query
...
# library. Detection points between stride positions stay unlabeled.

library_index = {subspace: index for index, subspace in enumerate(library)}
labeled_stop = extent_stop - WINDOW_LENGTH
labeled_points = np.arange(extent_start, labeled_stop, LABEL_STEP)
